...
async function callAI(messages, system, max = 400, geminiKey = '') {
  const res = await fetch('/api/ai', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ system, messages, max, ...(geminiKey && { geminiKey }) }),
  });
...
export default function Untangle() {
  ...
  const [hasKey, setHasKey] = useState(() => !!apiKey.get());
  ...
  const send = async () => {
    ...
    const reply = await callAI(newMessages, buildSystem(idol), 400, apiKey.get());
  ...
  if (phase === 'landing') return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '48px 16px', textAlign: 'center' }}>
      ...
      <button onClick={() => { apiKey.clear(); setHasKey(false); }} style={{
